...
async function getFilteredData() {
    ...
    try {
        ...
        const todayData = await getCollectionData("/homework/Non-fiction and Speaking/items", [todayFilter]);
        ...
        const waitingData = await getCollectionData("/homework/Non-fiction and Speaking/items", [
            fiveDaysAgoFilter,
            yesterdayFilter,
            statusWaitingFilter,
        ]);
...
async function renderData() {
    const nonfictionHomeworkList = document.getElementById("nonfictionHomework-list");

    try {
        const data = await getFilteredData();
        if (data.length === 0) {
            nonfictionHomeworkList.innerHTML = "<p>데이터가 없습니다.</p>";
            return;
        }

        nonfictionHomeworkList.innerHTML = ""; // 기존 내용 초기화
        data.forEach(item => {
            ...
            nonfictionHomeworkList.appendChild(div);
        });
    } catch (error) {
